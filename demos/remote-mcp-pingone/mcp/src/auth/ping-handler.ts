...
export const handleAuthorize = async (c: Context<{ Bindings: Env & { OAUTH_PROVIDER: OAuthHelpers } }>) => {
  try {
    ...
    const isClientApprovedCloudflare = await isClientApproved(c.req.raw, clientId, env.COOKIE_ENCRYPTION_KEY);
    if (isClientApprovedCloudflare) {
      ...
      const { stateToken } = await createOAuthState(extendedRequest, env.OAUTH_KV);
      const { setCookie: stateCookie } = await bindStateToSession(stateToken);
      return redirectToPingOne(extendedRequest, c.req.raw.url, stateToken, stateCookie);
    };
...
export const handleConsentApproval = async (c: Context<{ Bindings: Env & { OAUTH_PROVIDER: OAuthHelpers } }>) => {
  try {
    ...
    const consentCookie = await addApprovedClient(c.req.raw, mcpClientAuthReq.clientId, env.COOKIE_ENCRYPTION_KEY);
    return redirectToPingOne(extendedRequest, c.req.raw.url, stateToken, stateCookie, consentCookie);
...
export const handlePingOneCallback = async (c: Context<{ Bindings: Env & { OAUTH_PROVIDER: OAuthHelpers } }>) => {
  try {
    ...
    const stateResult = await validateOAuthState(c.req.raw, env.OAUTH_KV);
    ...
    const oidcClient = await initPingOneOidcClient();
    ...
    const userClaims = await fetchPingOneUserClaims(tokenSet.access_token);
