 ...
 */
const validatePingOneToken = async (env: Env, token: string): Promise<JWTPayload> => {
  const jwksUrl = new URL(env.API_ISSUER + '/jwks');
...
export const authenticationMiddleware = createMiddleware<{Bindings: Env, Variables: APIVariables}>(async (c, next) => {
  try {
    ...
    const payload = await validatePingOneToken(c.env, token);
